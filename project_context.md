...
```
recipes          — id, name, description, cook_time (total, start→table), prep_time (hands-on only, nullable),
                   difficulty, cuisine_type, is_vegetarian, is_vegan, is_side_dish, is_baking,
                   portions, is_freezable (default true), freezer_months (nullable THT override),
                   health_score/health_rationale/health_tip/health_scored_at (nullable), image_path (nullable), created_at
ingredients      — id, recipe_id, name, amount, unit, sort_order
steps            — id, recipe_id, sort_order, description, wait_time_minutes (nullable), track (main|meanwhile)
cook_sessions    — id, recipe_id, cooked_at, notes, cooked_by (michael|rachel, nullable),
                   cooking_mode (bool), current_step, step_started_at, timer_seconds, timer_started_at,
                   last_activity_at, group_id (nullable), active_seconds (nullable — wall-clock of a finished cook),
                   finished_at (nullable — NULL = still cooking)
step_durations   — recipe_id, track, sort_order, avg_seconds, stddev_seconds, sample_count (EWMA per step)
step_time_logs   — raw per-observation seconds, counted (NULL=pending / 1 / 0), keyed by (recipe_id, track, sort_order)
...
| GET    | /dashboard/status             | Read-only snapshot for external polling (active cook + today's planned meal), no auth |
...
- **Cooking mode** (`CookingMode.jsx`, route `/recipes/:id/cook`) fully replaces the old instant "Gekookt!" log — every cook session now goes through a guided flow: `navigator.wakeLock` keeps the screen on (re-requested on `visibilitychange` since the browser auto-releases it when a tab loses visibility), one recipe step per page with Next/Prev, a regex (`/\d+\s*(minuten|minuut|min)/i`) scans each step's text and offers a tappable timer suggestion (countdown driven by `Date.now()` math, not tick-counting, so backgrounding doesn't desync it — ends with a vibration + synthesized Web Audio beep), and a finish screen prompts for a photo before calling `POST /sessions/{id}/finish`.
  ...
  - The person cooking sees a "nog ~X min · klaar rond HH:MM" line in `CookingMode.jsx`; the other user sees the same estimate as an ambient banner (`ActiveSessionBanner.jsx`, polls `GET /sessions/active` every ~15s: "Koken van X in uitvoering — nog Y minuten"). Estimate math lives in `app/timing.py` (`estimate_remaining`): per step it uses the learned `step_durations` EWMA average if there is one, else the authored `steps.wait_time_minutes`, else a flat share of the remaining budget. Budget = the recipe's learned typical duration (median of recent `cook_sessions.active_seconds`, ≥2 samples) when available, else `cook_time*60`, else no estimate. `estimated_remaining_low/high_seconds` bracket it via the learned per-step spread. A running step timer's remaining seconds replace that step's share.
  - Step-time learning (`app/timing.py` + `routers/sessions.py`): each `advance_step`/`finish` logs `now − step_started_at` for the step being left. First observation is the baseline; within `max(2σ, 90s)` (or `max(15%, 90s)` before 4 samples) it folds into the EWMA silently; further off it's held as a pending `step_time_logs` row (`counted=NULL`) surfaced as `pending_step_confirmation` — shown in `CookingMode` as a *non-blocking* card that never covers the current step. `recompute_step_duration` replays the EWMA from the counted raw logs on every write (new sample / confirm / quit-undo) so it stays exact.
  - **While a timer is counting down, stepping through the recipe is a preview, not a move.** `_timer_is_live(row, now)` gates it: `advance_step` with a live timer just bumps `last_activity_at` — it does not move `current_step`, stop the timer, or log a step time. Frontend mirrors this (`CookingMode.jsx`'s `timerRunning` → local `stepIndex` only, an amber "je kijkt vooruit/terug" hint, the timer chip shows which step it belongs to, "Volgende" on the last step is disabled "Timer loopt nog"). When the timer hits 0 the view snaps back to `timerStepIndex`. A wait step (`wait_time_minutes` set) is also never logged unless its timer actually ran — otherwise the elapsed is just "time until the cook clicked past".
...
- **`GET /dashboard/status`** (added for an external polling dashboard, not used by the frontend itself): flat JSON, no auth — same no-auth trust model as the rest of the app, and the whole thing is already Tailscale-only, so a static-key header would just be theater. `updated_at` uses `datetime.now().astimezone()` (system-local, not hardcoded to a timezone) rather than the naive-UTC `datetime('now')` SQLite default some columns still use (see `cook_sessions.cooked_at`'s schema default) — do not copy that pattern here, it's the exact naive/aware mismatch that has caused bugs before. Numeric fields default to `0`/`""` instead of `null` when nothing's active/planned; `cooking_recipe_image_path` is the one exception — it's `null` unless a cook is active *and* that recipe has a photo, and holds the photo's `/uploads/...` path (same "latest photo across the recipe's cook sessions" rule as `RecipeOut.cover_photo`), served with a real `image/*` Content-Type by the static mount so a polling dashboard can `<img src>` it directly with no auth. `cook_time_remaining_seconds` is read straight from `sessions.active_session`'s `estimated_remaining_seconds` (called directly as a function, bypassing its `Depends` default, rather than duplicating the timer/step-share math) — that field already folds the current step's live timer into the total, so it must **not** be added on top of `active_timer_remaining_seconds` (that field alone is only the current *step's* countdown, not the whole cook's ETA — mixing the two double-counts). "Today" for the planned-recipe lookup reuses `planner._today()`/`_week_start()` via the module object (not a bound import) so the existing test-freezing convention (`monkeypatch.setattr(planner_module, "_today", ...)`) still works.

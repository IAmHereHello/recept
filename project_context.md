...
- **`GET /dashboard/status`** (added for an external polling dashboard, not used by the frontend itself): flat JSON, no auth — same no-auth trust model as the rest of the app, and the whole thing is already Tailscale-only, so a static-key header would just be theater. `updated_at` uses `datetime.now().astimezone()` (system-local, not hardcoded to a timezone) rather than the naive-UTC `datetime('now')` SQLite default some columns still use (see `cook_sessions.cooked_at`'s schema default) — do not copy that pattern here, it's the exact naive/aware mismatch that has caused bugs before. Numeric fields default to `0`/`""` instead of `null` when nothing's active/planned. `cook_time_remaining_seconds` is read straight from `sessions.active_session`'s `estimated_remaining_seconds` (called directly as a function, bypassing its `Depends` default, rather than duplicating the timer/step-share math) — that field already folds the current step's live timer into the total, so it must **not** be added on top of `active_timer_remaining_seconds` (that field alone is only the current *step's* countdown, not the whole cook's ETA — mixing the two double-counts). "Today" for the planned-recipe lookup reuses `planner._today()`/`_week_start()` via the module object (not a bound import) so the existing test-freezing convention (`monkeypatch.setattr(planner_module, "_today", ...)`) still works.
